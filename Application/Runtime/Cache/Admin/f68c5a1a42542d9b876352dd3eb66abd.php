...
<script type="text/javascript">
                      ...
                      function delAll(){
                          ...
                          if(confirm("确定要删除数据吗")){
								var idstr="";
								$(".del").each(function(i,x){
									if(x.checked){
										idstr=idstr+x.value+",";
									}
								})
							  
							 $.get("<?php echo U('Article/article_del_all');?>",{"ids":idstr},function(d){
							     if (d) {
                                    alert('删除成功');
								    location.href="<?php echo U('Article/article_list');?>";
							     }
			      ...
			      function datadel2(){
			      	    ...
			      	    $('input[class=del]').attr('checked',true);
			      ...
			      function datadel3(){
			             ...
			             var del3=$('tr input:checked');
			             $('input[class=del]').attr('checked',true);
			             del3.attr('checked',false);
	...
	<div class="Hui-article">
		...
		<article class="cl pd-20">
			<div class="cl pd-5 bg-1 bk-gray mt-20">
				<span class="l">
				<a href="javascript:vol()" onclick="delAll()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a>
				<a href="javascript:;" onclick="datadel2()" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe6e2;</i> 全选</a>
				<a href="javascript:;" onclick="datadel3()" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe6e2;</i> 反选</a>
				<a class="btn btn-primary radius" href="/tp/index.php/Admin/Article/article_add">添加文章</a>
			...
			<div class="mt-20">
				<table class="table table-border table-bordered table-bg table-hover table-sort">
					...
					<tbody>
					    <?php if(is_array($articles)): $i = 0; $__LIST__ = $articles;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
								<td><input type="checkbox" value="<?php echo ($vo["id"]); ?>" name="id" class="del"></td>
								<td><?php echo ($vo["id"]); ?></td>
								<td title="<?php echo ($vo["title"]); ?>"><a target="_blank" href="#" title="<?php echo ($vo["titles"]); ?>"><?php echo ($vo["title"]); ?></a></td>
								<td>
                                <?php if($vo['pic'] != ''): ?><img src="/tp<?php echo ($vo["pic"]); ?>" height="40" width="70">
                                <?php else: ?>
                                    暂无缩略图<?php endif; ?>
								</td>
								...
								<a onClick="return confirm('确定要删除栏目吗')" href="/tp/index.php/Admin/Article/article_del/id/<?php echo ($vo["id"]); ?>">删除</a>
								<a onClick="return confirm('确定要修改栏目吗')" href="/tp/index.php/Admin/Article/article_edit/id/<?php echo ($vo["id"]); ?>">修改</a>
							</td>
